...
class UploaderFileTypeValidator extends common_1.FileValidator {
    ...
    async isValid(file) {
        ...
        if (!this.validationOptions.acceptMimeType.includes(mime)) {
            await (0, promises_1.unlink)(parseFile.path);
            return false;
        }
        if (this.validationOptions.renameIfMimeWrong) {
            console.log('=====================Intercept Rename File If Mime Wrong');
            const name = (0, path_1.basename)(parseFile.filename, (0, path_1.extname)(parseFile.filename));
            const filename = `${name}.${ext}`;
            const path = `${parseFile.destination}/${filename}`;
            await (0, promises_1.rename)(parseFile.path, path);
